...
public class SWEA12372 {
    ...
    public static void main(String[] args) throws Exception {
        int TC;
        //System.setIn(new java.io.FileInputStream("./src/partial_sort_input.txt"));
...
class UserSolution12372 {
        ...
        while(current > 0 && users[current][1] >= users[(current - 1) / 2][1]) {
            if(users[current][1] == users[(current - 1) / 2][1]) {
                if(users[current][0] < users[(current - 1) / 2][0]) {
                    int tempId = users[(current - 1) / 2][0];
                    int tempIc = users[(current - 1) / 2][1];
                    users[(current - 1) / 2][0] = users[current][0];
                    users[(current - 1) / 2][1] = users[current][1];
                    users[current][0] = tempId;
                    users[current][1] = tempIc;
                    current = (current - 1) / 2;
                } else {
                    break;
                }
    ...
    public int[] pop() {
        int loop = userCount >= 10 ? 10 : userCount;
        int[] result = new int[10];
            ...
            while(current * 2 + 1 < heapSize) {
                ...
                if (current * 2 + 2 == heapSize)
                    ...
                    if(users[current * 2 + 1][1] == users[current * 2 + 2][1]) {
                        child = users[current * 2 + 1][0] < users[current * 2 + 2][0] ? current * 2 + 1 : current * 2 + 2;
                    } else {
                        child = users[current * 2 + 1][1] > users[current * 2 + 2][1] ? current * 2 + 1 : current * 2 + 2;
                    }
                }
